...
celltypes=rna3.obs["ATAC.Subtype.Apr19_2025"].unique().tolist()

for celltype in celltypes:
    print(celltype)
    tmp= rna3[rna3.obs["ATAC.Subtype.Apr19_2025"]==celltype].copy()
    # check if exist circadian genes, if not continue
    if not os.path.exists(f"/net/bmc-lab4/data/kellis/users/zunpeng/05_results/02_circadian/03_snMultiome_Tumor_Jan19_2024/01_snRNA_cellbender/01_map_snATAC_UMAP/CircadianGenes_v2_Subtype/Integration/{celltype}.discoODAres_CS_ANNOATION.outer.tsv"):
        print(f"No circadian genes for {celltype}, skipping...")
        continue
    circadian_genes= pd.read_csv(f"/net/bmc-lab4/data/kellis/users/zunpeng/05_results/02_circadian/03_snMultiome_Tumor_Jan19_2024/01_snRNA_cellbender/01_map_snATAC_UMAP/CircadianGenes_v2_Subtype/Integration/{celltype}.discoODAres_CS_ANNOATION.outer.tsv",
                            sep="\t",index_col=0)
    ...
    df2_sorted.to_csv(f"rna4.Cellbender.{celltype}.CircadianGenes.ZT4_10_16_22.top.sorted.tsv.gz", sep="\t", compression="gzip")
