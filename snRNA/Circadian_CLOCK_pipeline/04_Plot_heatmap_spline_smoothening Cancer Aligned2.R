...
# 定义一个函数计算圆均值（单位小时）
circ_mean_hours <- function(h1, h2, period = 24) {
  # 转成弧度
  rad1 <- circular(h1 / period * 2 * pi, units = "radians", modulo = "2pi")
  rad2 <- circular(h2 / period * 2 * pi, units = "radians", modulo = "2pi")
  
  mean_rad <- mean(c(rad1, rad2))  # circular 包的均值
  # 转回小时
  mean_hours <- (mean_rad %% (2 * pi)) * period / (2 * pi)
  return(mean_hours)
}
...
for (celltype in celltypes){
  ...
  # generate a new folder
    dir.create(paste0(celltype1,"_vs_",celltype, ".Aligned.smooth.spline2"), showWarnings = FALSE)
    setwd(paste0(celltype1,"_vs_",celltype, ".Aligned.smooth.spline2"))
  ...
  CircadianGenes <- CircadianGenes[CircadianGenes$phase_category=="Aligned", ]
  # Assign circadian phases (P1–P8)
# 3) 按行计算 ac1 与 ac2 的圆均值（新列 acrophase）
   CircadianGenes$acrophase <- mapply(circ_mean_hours,CircadianGenes$ac1,CircadianGenes$ac2)
 ...
 write.table(CircadianGenes, file = paste0(celltype1,"_with_",celltype, "_common_circadian_genes_withacrophasediff.csv"), sep = ",", row.names = TRUE, col.names = TRUE)
  ...
  pt.matrix1 <- apply(res1, 1, function(x) smooth.spline(x, df = 3, spar = 0.3)$y)
 ...
 res2 <- fread(paste0(matrixIndir,"rna4.Cellbender.",celltype, ".CircadianGenes.ZT4_10_16_22.top.sorted.tsv.gz"),
  ...
  # Smoothing and Z-score normalization
  pt.matrix2 <- apply(res2, 1, function(x) smooth.spline(x, df = 3, spar = 0.3)$y)
...
# 3) 按行计算 ac1 与 ac2 的圆均值（新列 acrophase）
 ...
 res2 <- fread(paste0(matrixIndir,"rna4.Cellbender.",celltype, ".CircadianGenes.ZT4_10_16_22.top.sorted.tsv.gz"),
  ...
  '''
smooth.spline 的平滑强度主要由 df（degrees of freedom）或 spar（smoothing parameter）控制。

df 越大 → 越接近原始数据 → 平滑程度减弱

spar 越小 → 越接近原始数据 → 平滑程度减弱（范围一般 0~1）
...
pt.matrix1 <- apply(res1, 1, function(x) smooth.spline(x, df = 8)$y)


df 越接近数据点数，曲线越贴合原始数据。

方法 2：手动设定 spar（更精细控制）

pt.matrix1 <- apply(res1, 1, function(x) smooth.spline(x, spar = 0.3)$y)


spar 越小，平滑越弱；可以尝试 0.2–0.5 之间。

方法 3：直接让它自动选择但加限制

pt.matrix1 <- apply(res1, 1, function(x) smooth.spline(x, spar = NULL, cv = TRUE)$y)


让函数用交叉验证选最佳平滑程度，但你可以之后手动调节范围。
  '''